...
for clf_ in clfs:
    for met in metrics:
        ...
        for i in range(topobalances.shape[-1]):
            ...
            im,cn = mne.viz.plot_topomap(topobalances[:,i],info,contours=0,extrapolate='head',axes=axes[i],cmap='jet',vmin=vmin,vmax=vmax,show=False)
        cbar_ticks = np.linspace(vmin,vmax,5)
        cbar=plt.colorbar(mappable=im,ax=axes,ticks=cbar_ticks,format='%.1f',location='bottom')
