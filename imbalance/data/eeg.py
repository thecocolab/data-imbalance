    ...
    for sub in sub_list:
        for task in task_list:
            raw = mne.io.read_raw_edf(
                filenames[sub][task], preload=False, verbose=False
            )
            ...
            psds, freqs = mne.time_frequency.psd_array_multitaper(
                epochs.get_data(), epochs.info["sfreq"], verbose=False
            )
...
def get_info(datapath="data"):
    os.makedirs(datapath, exist_ok=True)
    sub = 1
    filepath = mne.datasets.eegbci.load_data(
        sub, [2], path=datapath, update_path=False
    )[0]
...
if __name__ == "__main__":
    ...
    mne.viz.plot_sensors(
        info, sphere=sphere, pointsize=25, linewidth=0, block=True, show_names=roi
    )
